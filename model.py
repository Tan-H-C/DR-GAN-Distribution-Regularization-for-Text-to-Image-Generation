...
def encode_image_by_2timesxx(ndf):
...
class NEXT_STAGE_G(nn.Module):
    ...
    def forward(self, h_code, c_code, word_embs, mask, imgs, a, b, c, sent_emb):
...
class G_NET(nn.Module):
    ...
    def forward(self, z_code, sent_emb, word_embs, mask, imgs):
        """
            :param z_code: batch x cfg.GAN.Z_DIM
        ...
        if cfg.TREE.BRANCH_NUM > 1:
            h_code2, att1, loss1, I_P1, I_star1, I_image1  = \
                self.h_net2(h_code1, c_code, word_embs, mask, imgs[0], 64, 64, thc, sent_emb)
           ...
           # print(errorxx)
        if cfg.TREE.BRANCH_NUM > 2:
            h_code3, att2, loss2, I_P2, I_star2, I_image2 = \
                self.h_net3(h_code2, c_code, word_embs, mask, imgs[1], 64, 128, thc, sent_emb)
...
class D_NET64(nn.Module):
    ...
    def forward(self, x_var, sent_emb, word_embs, mask, noise):
...
def encode_image_by_1times(ndf):
    encode_img = nn.Sequential(
        # --> state size. ndf x in_size/2 x in_size/2
